...
case class ShutdownNode() extends GossipMessage
...
object project2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var failureProb: Double = args(3).toDouble
	writer.println("failureProb: "+failureProb)
    var numNodetoFail: Int = (numNodes.toDouble * failureProb).toInt
    ...
    if("gossip" == algorithm){
      for(i <-0 to numNodetoFail-1){
        nodes(Random.nextInt(nodes.size)) ! ShutdownNode()
      }
      var randNode: Int = Random.nextInt(nodes.size)
      while(nodes(randNode).isTerminated) {
        randNode = Random.nextInt(nodes.size)
      }
	  nodes(randNode) ! ReceiveGossip()
	  ...
	  nodes(randNode) ! ReceivePushSum(0, 0)
    }else{
